...
class PageCategory extends Component {
  ...
  handleCheckboxChange = (category, itemName) => {
  ...
  handlePublisherChange = (publisherId) => {
    this.setState((prevState) => ({
  ...
  loadMore = () => {
    this.setState((prevState) => ({
      productsToShow: prevState.productsToShow + this.state.productsToAdd,
    }));
  };
  render() {
    const { title, authors, books, genres, isLoading, languages, publishers } =
      this.props;
